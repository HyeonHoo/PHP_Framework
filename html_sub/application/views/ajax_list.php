...
<script>
       function find_text()
		{
			if (!form1.text1.value)      
				form1.action="/~sale53/ajax/lists";
			else                                   
				form1.action="/~sale53/ajax/lists/text1/" + form1.text1.value;
			form1.submit();
		}
					...
					complete: function(xhr, textStatus) {
						var no = xhr.responseText;
								...
								$("#table_list").append(
									"<tr id='rowno"+no+"'>"+
									"		<td>"+no+"</td>"+
									"		<td><a href='/~sale53/ajax/view/no/"+no+"<?=$tmp; ?>'>"+name+"</a></td>"+
		...
		<form name="form1" action="" method="post">
			<div class="row">
				...
				<div class="col-9" align="right">   
						<a href="#collapseExample" class="btn btn-sm mycolor1"
							data-toggle="collapse" aria-expanded="false" aria-controls="collapseExample">추가</a>
				</div>
			</div>
		</form>
	...
	<table class="table  table-bordered  table-sm  mymargin5" id="table_list">
		...
		<tr id="rowno<?=$no; ?>">
			<td><?=$no; ?></td>
			<td><a href="/~sale53/ajax/view/no/<?=$no;?> <?=$tmp; ?>"><?=$row->name53; ?></a></td>
			<td>
