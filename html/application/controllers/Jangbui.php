<?
    class Jangbui extends CI_Controller {       // jangbui클래스 선언
		...
		redirect("/~sale53/jangbui/lists" . $text1 . $page);
		}
		...
		public function add()
		{
			...
			else              // 입력화면의 저장버튼 클릭한 경우
			{
				$data = array(
					...
					"numi53"			 => $this->input->post("numi",TRUE),
					"numo53"		 => 0,
				...
				redirect("/~sale53/jangbui/lists" . $text1 . $page);    //   목록화면으로 이동.
			}
		}
		...
		public function edit()
		{
			...
			if ( $this->form_validation->run()==FALSE )
			{
				...
				$this->load->view("main_header");
				$data["row"]=$this->jangbui_m->getrow($no);
				$this->load->view("jangbui_edit",$data);    // 입력화면 표시
				$this->load->view("main_footer");
