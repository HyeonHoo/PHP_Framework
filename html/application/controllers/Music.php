    ...
    class Music extends CI_Controller {       // music클래스 선언
        ...
        public function lists()
        {
			...
			$text1 = array_key_exists("text1",$uri_array) ? urldecode($uri_array["text1"]) : "" ;
					...
					$config["total_rows"] = $this->music_m->rowcount($text1);  // 전체 레코드개수 구하기
            ...
            $data["list"] = $this->music_m->getlist($text1,$start,$limit);    // 자료읽어 data배열에 저장 
		...
		redirect("/~sale53/music/lists" . $text1 . $page);
		}
	      ...
	      public function add()
      {
         ...
         $this->form_validation->set_rules("genre_no","장르명","required");
         $this->form_validation->set_rules("name","이름","required|max_length[50]");
         

         if ( $this->form_validation->run()==FALSE )
         {
            $data["list"] = $this->music_m->getlist_genre();
         ...
         {
            $data = array(
               "genre_no53"    => $this->input->post("genre_no",TRUE),
            ...
            $music_file=$this->call_upload2();                  // 업로드 시작
            if ($music_file) $data["music_file53"]=$music_file;               // 파일이름 저장

            $result = $this->music_m->insertrow($data); 

            redirect("/~sale53/music/lists" . $text1 . $page);    //   목록화면으로 이동.
         }
      }
      ...
      public function edit()
      {
         ...
         if ( $this->form_validation->run()==FALSE )
         {
            ...
            $this->load->view("main_header1");
            $data["row"]=$this->music_m->getrow($no);
            $this->load->view("music_edit",$data);    // 입력화면 표시
            $this->load->view("main_footer1");
