<?
    class Jaegoi extends CI_Controller {       // jaegoi클래스 선언
        function __construct()                           // 클래스생성할 때 초기설정
        {
            parent::__construct();
            $this->load->database();                     // 데이터베이스 연결
            $this->load->model("jaegoi_m");    // 모델 jaegoi_m 연결
        ...
        public function lists()
        {
			...
			$text2 = array_key_exists("text2",$uri_array) ? urldecode($uri_array["text2"]) : date("Y-m-d");
					...
					$base_url = "/jaegoi/lists/text1/$text1/text2/$text2/page";    // $page_segment = 6;

					$page_segment = substr_count( substr($base_url,0,strpos($base_url,"page")) , "/" )+1;
					$base_url = "/~sale53" . $base_url;

					$config["per_page"]	 = 5;                              // 페이지당 표시할 line 수
					$config["total_rows"] = $this->jaegoi_m->rowcount($text1,$text2,);  // 전체 레코드개수 구하기
			...
			$data["text2"]=$text2;
            $data["list"] = $this->jaegoi_m->getlist($text1,$text2,$start,$limit);    // 자료읽어 data배열에 저장 
            $this->load->view("main_header1");						 // 상단출력(메뉴)
            $this->load->view("jaegoi_list",$data);				 // jaegoi_list에 자료전달
		...
		$data["text2"]=$text2;
		$data["page"]=$page;
		$data["row"] = $this->jaegoi_m->getrow($no);

		$this->load->view("main_header1");                  
        $this->load->view("jaegoi_view",$data); 
		...
		$this->jaegoi_m->deleterow($no);

		redirect("/~sale53/jaegoi/lists" . $text1 . $text2 . $page);
		}
		...
		$text1 = array_key_exists("text1",$uri_array) ? "/text1/" . urldecode($uri_array["text1"]) : "" ;
			...
			{
				$data["list"] = $this->jaegoi_m->getlist_music();
				$this->load->view("main_header1");
				$this->load->view("jaegoi_add",$data);    // 입력화면 표시
				$this->load->view("main_footer1");
			...
			else              // 입력화면의 저장버튼 클릭한 경우
			{
				$data = array(
					...
					"mui53"			 => $this->input->post("mui",TRUE),
					"muo53"		 => 0,
				...
				$result = $this->jaegoi_m->insertrow($data); 

				redirect("/~sale53/jaegoi/lists" . $text1 . $text2 . $page);    //   목록화면으로 이동.
			}
		}
		...
		public function edit()
		{
			...
			if ( $this->form_validation->run()==FALSE )
			{
				...
				$this->load->view("main_header1");
				$data["row"]=$this->jaegoi_m->getrow($no);
				$this->load->view("jaegoi_edit",$data);    // 입력화면 표시
				$this->load->view("main_footer1");
			...
			else              // 입력화면의 저장버튼 클릭한 경우
			{
				...
				$result = $this->jaegoi_m->updaterow($data,$no); 
