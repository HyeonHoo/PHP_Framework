<?
    class Chartlist extends CI_Controller {       // chartlist클래스 선언
        function __construct()                           // 클래스생성할 때 초기설정
        {
            parent::__construct();
            $this->load->database();                     // 데이터베이스 연결
            $this->load->model("chartlist_m");    // 모델 chartlist_m 연결
        ...
        public function lists()
        {
			...
			$base_url="/chartlist/lists/text1/$text1/text2/$text2/page";
					...
					$config["total_rows"] = $this->chartlist_m->rowcount($text1,$text2);  // 전체 레코드개수 구하기
			...
			$data["text2"]=$text2;
            $data["list"] = $this->chartlist_m->getlist($text1,$text2,$start,$limit);    // 자료읽어 data배열에 저장 

            $this->load->view("main_header1");						 // 상단출력(메뉴)
            $this->load->view("chartlist_list",$data);				 // chartlist_list에 자료전달
